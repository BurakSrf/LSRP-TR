...
Server:CheckAccount(playerid, race)
{
    ...
    new query[455];
    mysql_format(m_Handle, query, sizeof(query), "SELECT id, active_id, adminlevel, adminname, testerlevel, testername, name FROM accounts WHERE name = '%e'", ReturnName(playerid));
...
Server:OnVehicleUpgrade(playerid, vehicleid, price, to_level)
{
    if(!IsPlayerConnected(playerid))
    {
