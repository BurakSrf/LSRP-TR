...
{
	if(PlayerData[playerid][pBrutallyWounded]) return SendErrorMessage(playerid, "Bu animasyonu şu anda kullanamazsın.");
	if(!IsPlayerInAnyVehicle(playerid)) return SendErrorMessage(playerid, "Bu animasyonu şu anda kullanamazsın.");
...
{
	if(!AnimationCheck(playerid)) return SendErrorMessage(playerid, "Bu animasyonu şu anda kullanamazsın.");
...
CMD:crossroad(playerid, params[])
{
	...
	new animid;
   	if(sscanf(params,"d",animid)) return SendServerMessage(playerid, "/crossroad [1 (klasik), 2 (kadın), 3 (çete), 4 (yaşlı)]");
	switch(animid)
	{
        ...
        case 4: PlayAnimation(playerid, "PED", "roadcross_old", 4.0, 0, 0, 0, 1, 1, 1);
  		default: SendServerMessage(playerid, "/crossroad [1 (klasik), 2 (kadın), 3 (çete), 4 (yaşlı)]");
   	}
	return 1;
}
...
CMD:animboz(playerid, params[])
{
	if(GetPlayerSpecialAction(playerid) == SPECIAL_ACTION_CUFFED)
		return SendServerMessage(playerid, "Kelepçeliyken bu komutu kullanamazsınız.");
...
CMD:lowrider(playerid, params[])
{
	...
	switch(id)
	{
		...
		default: SendServerMessage(playerid, "/lowrider [1-39]");
	}
	return 1;
}
