/*
        Los Santos Roleplay v2 by Amper, Renard (mortale) and Haroldfinch
*/
...
#include "lsrp_globals.pwn"
#include "lsrp_anticheat.pwn"
#include "lsrp_callbacks.pwn"
#include "lsrp_acmds.pwn"
#include "lsrp_pcmds.pwn"
#include "lsrp_animations.pwn"
#include "lsrp_dialogs.pwn"
#include "lsrp_poker.pwn"
#include "lsrp_functions.pwn"
#include "lsrp_maps.pwn"
//#include "lsrp_mdc.pwn"
...
//  g_WelcomeChannelId = DCC_FindChannelById("700677506616918017"); // test
    ...
    //Textlabel
    CreateDynamic3DTextLabel("{FFFF00}/satinal\n{FFFFFF}yazarak kýyafet alabilirsin!", COLOR_WHITE, 1102.7249, -1440.0239, 15.7969, 15.0);
    ...
    CreateDynamicPickup(1239, 1, 675.5908, -2540.1199, 13.4365);
    CreateDynamic3DTextLabel("/meslek kamyoncu", -1, 675.5908, -2540.1199, 13.4365, 1.5);
    return 1;
}
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == DIALOG_SELECT_SKIN)
    {
        if(response)
        {
            ...
            PlayerTutorialTimer[playerid] = SetTimerEx("SetTutorial", 1000, false, "i", playerid);
        }
        else Kick(playerid);
    }
        ...
        case DIALOG_DEALERSHIP_PURCHASE:
        {
            ...
            else
            {
                ...
                if(vehicleid != INVALID_VEHICLE_ID)
                {
                    ...
                    CarData[vehicleid][carImpounded] = 0;
        ...
        case DIALOG_ADMIN_NAME:
        {
            if(response)
            {
                if(strlen(inputtext) > 60)
                    return ShowPlayerDialog(playerid, DIALOG_ADMIN_NAME, DIALOG_STYLE_INPUT, "Los Santos Roleplay: Yönetici Adý", "Bir yönetici olarak görülecek adýnýz sisteme girilmemiþ.\nYönetici adýnýzý girin:", "Tamam", "Kapat");
...
//                EnableAntiCheat(listitem, IsAntiCheatEnabled(listitem) ? 0 : 1);
        ...
        case DIALOG_PASSWORD_CHANGE:
        {
            if(response)
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_USE, DIALOG_STYLE_MSGBOX, "Los Santos Roleplay: Þifre Deðiþtir", "Þifreniz baþarýyla deðiþtirildi.", "Tamam", "");
            }
...
public OnPlayerEditAttachedObject(playerid, response, index, modelid, boneid, Float:fOffsetX, Float:fOffsetY, Float:fOffsetZ, Float:fRotX, Float:fRotY, Float:fRotZ, Float:fScaleX, Float:fScaleY, Float:fScaleZ)
{
    ...
    if(EditingDisplay[playerid])
    {
        ...
        if (response)
        {
            ...
            mysql_format(m_Handle, query, sizeof(query), "INSERT INTO weapon_attachments (playerdbid, WeaponID, BoneID, PosX, PosY, PosZ, RotX, RotY, RotZ) VALUES (%i, %i, %i, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f) ON DUPLICATE KEY UPDATE PosX = VALUES(PosX), PosY = VALUES(PosY), PosZ = VALUES(PosZ), RotX = VALUES(RotX), RotY = VALUES(RotY), RotZ = VALUES(RotZ)", PlayerData[playerid][pSQLID], weaponid, WeaponSettings[playerid][enum_index][WeaponBone], fOffsetX, fOffsetY, fOffsetZ, fRotX, fRotY, fRotZ);
            mysql_tquery(m_Handle, query);
        }
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
    ...
    if(newstate==PLAYER_STATE_PASSENGER) // deag/mp5 bugfix
    {
        new gun,tmp;
...
public OnPlayerClickPlayerTextDraw(playerid, PlayerText:playertextid)
{
    ...
    /*
    if(GetPVarInt(playerid, "Viewing_MDC"))
    {
        if(playertextid == MDC_UI[playerid][9]) // kapat
        {
            MDC_Toggle(playerid, false);
        }
    }
    */
    ...
    if(GetPVarInt(playerid, "Viewing_CharacterList"))
    {
        for(new x = 0; x < 3; x++)
        {
            ...
            {
                printf("sqlid: %i", CharacterHolder[playerid][x]);
                Selection_Hide(playerid);
...
public OnPlayerRequestClass(playerid, classid)
{
    if(pLoggedIn[playerid])
    {
        if(PlayerData[playerid][pBrutallyWounded])
        {
            ...
            SetTimerEx("SecureSpawn", 3000, false, "i", playerid);
        }
    }
    return 1;
}
...
public OnPlayerEnterDynamicArea(playerid, areaid)
{
    ...
    if(GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
    {
        ...
        if(GetPVarInt(playerid, "AtSpike") != -1)
        {
            new s = GetPVarInt(playerid, "AtSpike");
            if(RoadblockData[s][RoadblockSpikes])
            {
                ...
                if(IsValidDynamicObject(RoadblockData[s][RoadblockObject])) DestroyDynamicObject(RoadblockData[s][RoadblockObject]);
                if(IsValidDynamicArea(RoadblockData[s][RoadblockAreaID])) {
                    Streamer_RemoveArrayData(STREAMER_TYPE_AREA, RoadblockData[s][RoadblockAreaID], E_STREAMER_PLAYER_ID, s);
                    DestroyDynamicArea(RoadblockData[s][RoadblockAreaID]);
                }

                RoadblockData[s][RoadblockSpikes] = false;
                Iter_Remove(Roadblocks, s);
            }
        }
    }
    ...
    if(GetPlayerMenu(playerid) == main_page)
    {
        switch(row)
        {
            ...
            case 6:
            {
                ...
                SendClientMessage(playerid, COLOR_WHITE, "forum.ls-rp.web.tr");
                SendClientMessage(playerid, COLOR_DARKGREEN, "____________________");
                ShowMenuForPlayer(main_page, playerid);
            }
        }
