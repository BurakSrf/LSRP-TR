...
{
	if(PlayerData[playerid][pAdmin] != 4000)
		return 0;
...
CMD:skick(playerid, params[])
{
	...
	SendServerMessage(id, "%s tarafýndan oyundan %s sebebiyle atýldýnýz.", ReturnName(playerid, 1), sebep);

	if(strlen(sebep) > 55)
	{
		adminWarn(1, sprintf("[GIZLI] %s isimli oyuncu %s tarafýndan atýldý. Sebep: %.55s", ReturnName(id, 1), ReturnName(playerid, 1), sebep));
		adminWarn(1, sprintf("...%s", sebep[55]));
	}
	else adminWarn(1, sprintf("[GIZLI] %s isimli oyuncu %s tarafýndan atýldý. Sebep: %s", ReturnName(id, 1), ReturnName(playerid, 1), sebep));
...
CMD:sjail(playerid, params[])
{
	...
	SendServerMessage(id, "%s tarafýndan %s sebebiyle %d dakika hapise gönderildiniz.", ReturnName(playerid, 1), reason, minutes);

	if(strlen(reason) > 45)
	{
		adminWarn(1, sprintf("[GIZLI] %s isimli oyuncu %s tarafýndan %d dakika hapise atýldý. Sebep: %.45s", ReturnName(id, 1), ReturnName(playerid, 1), minutes, reason));
		adminWarn(1, sprintf("...%s", reason[45]));
	}
	else adminWarn(1, sprintf("[GIZLI] %s isimli oyuncu %s tarafýndan %d dakika hapise atýldý. Sebep: %s", ReturnName(id, 1), ReturnName(playerid, 1), minutes, reason));
...
CMD:arealookup(playerid, params[])
{
	...
	foreach(new i : Player)
	{
		...
		{
			SendClientMessageEx(playerid, COLOR_GREY, "%s[ID: %i] was found", ReturnName(i), i);
		}
	}
	return 1;
}
...
CMD:nearme(playerid, params[])
{
	...
	foreach(new i : Player)
	{
		...
		{
			SendClientMessageEx(playerid, COLOR_GREY, "%s[ID: %i] Is Masked: %s Car ID: %i", ReturnName(i), i, (!PlayerData[i][pMaskOn]) ? ("No") : ("Yes"), GetPlayerVehicleID(i));
		}
	}
...
CMD:clearcolour(playerid, params[])
{
	...
	format(str, sizeof(str), "AdmCmd: %s has cleared %s' name colour.", ReturnName(playerid), ReturnName(id));
...
CMD:checkfreq(playerid, params[])
{
	...
	SendClientMessageEx(playerid, COLOR_ADM, "Players in frequency %d:", channel);
	foreach(new i : Player)
	{
...
CMD:getname(playerid, params[])
{
	...
	if(dbid < 1)return SendClientMessage(playerid, COLOR_ADM, "HATA:{FFFFFF} You can't go below one.");
...
CMD:setjob(playerid, params[])
{
	...
	if(!IsPlayerConnected(id)) return SendErrorMessage(playerid, "Belirttiðiniz kiþi oyunda yok.");
	if(!pLoggedIn[id]) return SendErrorMessage(playerid, "Belirttiðiniz kiþi henüz þifresini girmemiþ.");
	if(job_id < 1 || job_id > 5) return SendErrorMessage(playerid, "Hatalý meslek ID girdin.");
...
CMD:setfightstyle(playerid, params[])
{
	...
	if(playerb != playerid) SendClientMessageEx(playerb, COLOR_ADM, "Your fightstyle was changed by %s.", ReturnName(playerid));
	return 1;
}
...
CMD:callpaycheck(playerid, params[])
{
	...
	adminWarn(4, sprintf("%s called a paycheck before the hour.", ReturnName(playerid, 1)));
...
CMD:givemoney(playerid, params[])
{
	...
	//WriteLog("admin_logs/kane/givemoney_log.txt", "[%s] %s gave $%s to %s", ReturnDate(), ReturnName(playerid, 1), MoneyFormat(give_amount), ReturnName(playerb, 1));
	return true;
}
...
CMD:graffitiperm(playerid, params[])
{
	...
	if(sscanf(params, "u", user))
		return SendUsageMessage(playerid, "/graffitiperm [karakter id/isim]");

	if(!pLoggedIn[user])
		return SendErrorMessage(playerid, "Girdiðin oyuncu ID veya isim aktif deðil.");

	GivePlayerWeapon(playerid, 42, 5000);

	if(PlayerData[playerid][pTester] && !PlayerData[playerid][pAdmin])
	{
		adminWarn(1, sprintf("Tester %s %s adlý kiþiye graffiti yetkisi verdi.", ReturnName(playerid), ReturnName(user)));
	}

	SendClientMessageEx(user, COLOR_YELLOW, "[!] %s sana graffiti yetkisi verdi. /graffiti.", ReturnName(playerid, 1));
	SendClientMessageEx(playerid, COLOR_YELLOW, "[!] %s isimli kiþiye graffiti yetkisi verdin.", ReturnName(user));
