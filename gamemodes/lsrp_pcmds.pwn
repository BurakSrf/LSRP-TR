...
CMD:giris(playerid, params[])
{
    ...
    if((h = IsPlayerNearProperty(playerid)) != -1)
    {
        ...
        PropertyData[h][PropertyExitWorld] = 100+PropertyData[h][PropertyID];
    ...
    if((b = IsPlayerNearBusiness(playerid)) != -1)
    {
        ...
        BusinessData[b][ExitWorld] = 10000+BusinessData[b][BusinessID];
    ...
    if(GetPVarInt(playerid, "AtGarage") != -1)
    {
        ...
        if(IsPlayerInRangeOfPoint(playerid, 3.0, GarageData[g][GaragePos][0], GarageData[g][GaragePos][1], GarageData[g][GaragePos][2]))
        {
            ...
            GarageData[g][ExitWorld] = GarageData[g][GarageID] + 5000;
    ...
    if(GetPVarInt(playerid, "AtPayNSpray") != -1)
    {
        ...
        /*  new panels, doors, lights, tires, Float: vehiclehealth, panelsp, doorsp, panelprice, doorprice, tireprice, tiresp, engineprice, Float:vehhp, price;
            ...
            new vehicleid = GetPlayerVehicleID(playerid);
            SetVehicleHealth(vehicleid, GetVehicleCondition(vehicleid, 0));
...
{
    if (PlayerData[playerid][pBrutallyWounded]) return SendServerMessage(playerid, "Bu komutu yaralý durumdayken kullanamazsýn.");
...
{
    if(IsPlayerInRangeOfPoint(playerid, 4.0, 1097.7529,-1439.8232,15.7981))
    {
        if (PlayerData[playerid][pBuyingClothing]) return SendClientMessage(playerid, COLOR_ADM, "ERISIM RED EDILDI:{FFFFFF} Zaten aksesuar düzenliyorsun.");
        Clothing_List(playerid, 1);
        return 1;
    }

    if(IsPlayerInRangeOfPoint(playerid, 4.0, 1092.4302,-1507.7614,15.7981))
    {
        if (PlayerData[playerid][pBuyingClothing]) return SendClientMessage(playerid, COLOR_ADM, "ERISIM RED EDILDI:{FFFFFF} Zaten aksesuar düzenliyorsun.");
        Clothing_List(playerid, 2);
        return 1;
    }

    if(IsPlayerInRangeOfPoint(playerid, 4.0, 1112.6389,-1527.6321,15.7981))
    {
        if (PlayerData[playerid][pBuyingClothing]) return SendClientMessage(playerid, COLOR_ADM, "ERISIM RED EDILDI:{FFFFFF} Zaten aksesuar düzenliyorsun.");
        Clothing_List(playerid, 3);
        return 1;
    }

    if(IsPlayerInRangeOfPoint(playerid, 4.0, 1143.9175,-1526.6506,15.7981))
    {
        if (PlayerData[playerid][pBuyingClothing]) return SendClientMessage(playerid, COLOR_ADM, "ERISIM RED EDILDI:{FFFFFF} Zaten aksesuar düzenliyorsun.");
        Clothing_List(playerid, 4);
        return 1;
    }

    if(IsPlayerInRangeOfPoint(playerid, 4.0, 1092.7722,-1524.8140,22.7397))
    {
        if (PlayerData[playerid][pBuyingClothing]) return SendClientMessage(playerid, COLOR_ADM, "ERISIM RED EDILDI:{FFFFFF} Zaten aksesuar düzenliyorsun.");
        Clothing_List(playerid, 5);
        return 1;
    }
...
//  LogPlayerAction(playerid, sprintf("%i numaralý aracý kiraladý.", RentalData[vehicleid][rentalcarSQLID]));
    return 1;
}
...
CMD:arac(playerid, params[])
{
    ...
    if(sscanf(params, "s[60]S()[128]", a_string, b_string))
    {
        ...
        SendClientMessage(playerid, COLOR_ACTION, "[Ýpucu] forum.ls-rp.web.tr adresinde butün bu eylemleri anlatan bir konu bulunuyor.");
    ...
    else if(!strcmp(a_string, "gelistir"))
    {
        ...
        if(GetPVarInt(playerid, "AtBusiness") == -1) return SendErrorMessage(playerid, "Yakýnýnda galeri yok.");
        if(BusinessData[GetPVarInt(playerid, "AtBusiness")][BusinessType] != BUSINESS_DEALERSHIP) return SendErrorMessage(playerid, "Bu iþyeri galeri deðil.");
        ...
        if(sscanf(b_string, "s[7]I(-1)S()[6]", type, type_level, confirm))
        {
            ...
            //SendClientMessage(playerid, COLOR_ACTION, "[TÝPLER] kilit, alarm, immob, sigorta, armour");

            //SendClientMessage(playerid, COLOR_ACTION, "[REPLACE] batarya, motor");
        ...
        if(!strcmp(type, "kilit"))
        {
            ...
            if(!isnull(confirm) && !strcmp(confirm, "onay", true))
            {
                ...
                VehicleUpgradeSign[vehicleid] = CreateDynamic3DTextLabel("(( | ))\nKÝLÝT YÜKSELTÝLÝYOR", COLOR_DARKGREEN, 0.0, 0.0, 0.0, 3.0);
                ...
                PlayerUpgradeTimer[playerid] = SetTimerEx("OnVehicleUpgrade", 4500, true, "iiii", playerid, vehicleid, total_cost, type_level);
            }
            else
            {
    ...
    else if(!strcmp(a_string, "kayitsil"))
    {
        ...
        format(CarData[vehicleid][carPlates], 32, " ");
...
CMD:unimpound(playerid, params[])
{
    ...
    if(PlayerData[playerid][pSQLID] != CarData[vehicleid][carOwnerID])
        return SendClientMessage(playerid, COLOR_ADM, "SERVER: Bu araca sahip deðilsin.");

    CarData[vehicleid][carImpounded] = -1;
    GiveMoney(playerid, -500);

    SendClientMessage(playerid, COLOR_DARKGREEN, "Aracýnýzý.");
    Car_Save(vehicleid);
    return 1;
}
...
CMD:aksesuar(playerid, params[])
{
    ...
    if (sscanf(params, "s[40]S()[90]", a_string, b_string))
    {
        ...
        {
            format(sub, sizeof(sub), "{FFFF00}%d. {FFFFFF}%s (Index: %d)\n", i+1, ClothingData[playerid][i][ClothingName], ClothingData[playerid][i][ClothingSlotID]);
            strcat(primary, sub);
            count++;
        }

        if(!count) return SendClientMessage(playerid, COLOR_ADM, "[!] {FFFFFF}Hiç aksesuarýn yok.");
        else ShowPlayerDialog(playerid, DIALOG_CLOTHING_MENU, DIALOG_STYLE_LIST, "Aksesuar Listesi", primary, "Seç", "<< Kapat");
    }

    SendUsageMessage(playerid, "/aksesuar giy | duzenle | sil | ver | sifirla");


/*  new id;

    if(!strcmp(a_string, "giy") || !strcmp(a_string, "g"))
    {
        if (sscanf(b_string, "d", id)) return SendUsageMessage(playerid, "/aksesuar giy [slot]");
        if (id < 1 || id >= MAX_CLOTHING_ITEMS) return SendServerMessage(playerid, "Geçersiz slot.");
        if (!ClothingData[playerid][id][attachment_SQLID]) return SendServerMessage(playerid, "Bu slotta aksesuarýn yok.");
        ...
        if (clothing_id == -1) return SendServerMessage(playerid, "Aksesuarý giyebilmek için ilk önce üstündekilerden bir kaçýný çýkar.");

        if (IsPlayerAttachedObjectSlotUsed(playerid, clothing_id))
        {
            SendClientMessageEx(playerid, COLOR_WHITE, "SERVER: %s aksesuarýný çýkarttýn.", ClothingData[playerid][id][attachment_name]);
            RemovePlayerAttachedObject(playerid, clothing_id);
        }
        else {
            SetPlayerAttachedObject(playerid, clothing_id, ClothingData[playerid][id][attachment_modelid], ClothingData[playerid][id][attachment_bone], ClothingData[playerid][id][attachment_Pos][0], ClothingData[playerid][id][attachment_Pos][1], ClothingData[playerid][id][attachment_Pos][2],
            ...
            SendClientMessageEx(playerid, COLOR_WHITE, "SERVER: %s aksesuarýný giydin.", ClothingData[playerid][id][attachment_name]);
        }
    }
    else if(!strcmp(a_string, "duzenle") || !strcmp(a_string, "d"))
    {
        ...
        new clothing_id = FreeAttachmentSlot(playerid);
        if (clothing_id == -1) return SendServerMessage(playerid, "Aksesuarýný düzenleyebilmek için ilk önce üstündekilerden bir kaçýný çýkar.");
    ...
    else if(!strcmp(a_string, "sil"))
    {
        ...
        SendClientMessageEx(playerid, COLOR_WHITE, "SERVER: %s aksesuarýný sildin.", ClothingData[playerid][id][attachment_name]);
...
CMD:modifiye(playerid, params[])
{
    ...
    SendClientMessage(playerid, COLOR_WHITE, "{FF6347}~k~~VEHICLE_LOOKLEFT~ {FFFFFF}ve {FF6347}~k~~VEHICLE_LOOKRIGHT~ {FFFFFF}tuþlarýyla kategorileri, {FF6347}~k~~VEHICLE_FIREWEAPON~ {FFFFFF}ve {FF6347}~k~~PED_LOCK_TARGET~ {FFFFFF}tuþlarýyla çeþitleri deðiþtirebilirsin.");
    return 1;
}
...
CMD:motor(playerid, params[])
{
    ...
    if(IsValidPlayerCar(vehicleid))
    {
        ...
        if(PlayerData[playerid][pSQLID] != CarData[vehicleid][carOwnerID] && PlayerData[playerid][pCarKey] != CarData[vehicleid][carID])
        {
            ...
            if(CarData[vehicleid][carTweak])
            {
                CarData[vehicleid][carRev] = gettime();
                SendServerMessage(playerid, "Motor kötü bir þekilde hasar aldý.");
                SendClientMessage(playerid, COLOR_YELLOW, "ÝPUCU: {FFFFFF}W ve S{FFFF00} tuþuna basarak motoru zorlayabilirsin.");
...
CMD:isbasi(playerid, params[])
{
	...
	else if(IsMedicFaction(playerid))
	{
	    if(!PlayerData[playerid][pMEDduty])
	    {
			...
			SendNearbyMessage(playerid, 20.0, COLOR_EMOTE, "* %s dolaptan ekipmanlarýný alýr.", ReturnName(playerid, 0));
			SetPlayerHealth(playerid, PlayerData[playerid][pMaxHealth]);
            GivePlayerWeapon(playerid, WEAPON_FIREEXTINGUISHER, 5000);
		}
		else
		{
...
CMD:samaritan(playerid, params[])
{
    SendClientMessage(playerid, COLOR_YELLOW, "SMS: Ellerimin baðlý olduðu dünya da yok ettiðim bir AI., Gönderen: Makine");
    return 1;
}

CMD:makine(playerid, params[])
{
    SendClientMessage(playerid, COLOR_YELLOW, "SMS: Çok fazla þey bilmek istiyorsun., Gönderen: Makine");
    return 1;
}

CMD:johnreese(playerid, params[])
{
    SendClientMessage(playerid, COLOR_YELLOW, "SMS: Jessica, onu sevdiðini biliyorum., Gönderen: Makine");
    return 1;
}

CMD:haroldfinch(playerid, params[])
{
    SendClientMessage(playerid, COLOR_YELLOW, "SMS: Los Santos, 24 yönünden kuzey, 32 yönünden güney., Gönderen: Makine");
    return 1;
}

CMD:gratisegit(playerid, params[])
{
	SendClientMessage(playerid, COLOR_YELLOW, "SMS: Görkem Büyüklimanlý hoþgeldiniz, kapýda beklediðiniz için üzgünüz. Gönderen: Gratis Kadýköy Þubesi");
	return 1;
}
...
CMD:sifredegistir(playerid, params[])
{
	if(CanChangePassword[playerid])
	{
		...
		strcat(largeString, "\twww.ls-rp.web.tr forum hesabýnýzla ayný þifreyi kullanmayýn.\n\n");
		strcat(largeString, "Unutmayýn, Los Santos Roleplay ekibi asla þifrenizi istemez.\n\n");
		strcat(largeString, "{F81414}Þifreniz en az 3 fazla 128 karakter aralýðýnda olmalýdýr.");
		ShowPlayerDialog(playerid, DIALOG_PASSWORD_CHANGE, DIALOG_STYLE_PASSWORD, "Los Santos Roleplay: Þifre Deðiþtir", largeString, "Deðiþtir", "Ýptal");
		return 1;
	}
	else
	{
		...
		if(strcmp(ReturnGPCI(playerid), HWID, false))
		{
			...
			strcat(largeString, "Lütfen istekte bulunmak ve þifre deðiþikliðine izin vermek için www.ls-rp.web.tr forumunu ziyaret edin.");
			ShowPlayerDialog(playerid, DIALOG_USE, DIALOG_STYLE_MSGBOX, "Los Santos Roleplay: Þifre Deðiþtir", largeString, "Tamam", "");
			return 1;
		}
		...
		if(strcmp(ReturnIP(playerid), RegisterIP, false))
		{
			...
			ShowPlayerDialog(playerid, DIALOG_PASSWORD_SECURITY_WORD, DIALOG_STYLE_PASSWORD, "Los Santos Roleplay: Þifre Deðiþtir", largeString, "Deðiþtir", "Ýptal");
			return 1;
		}
...
CMD:kurallar(playerid, params[])
{
	...
	SendClientMessage(playerid, -1, "forum.ls-rp.web.tr");
	return 1;
}
	...
	if((biz = IsPlayerInBusiness(playerid)) != -1)
	{
		if(BusinessData[biz][BusinessType] == BUSINESS_CLOTHING)
		{
			if(PlayerData[playerid][pGender] == 0)
			{
			    ...
			    return ShowPlayerDialog(playerid, DIALOG_SELECT_SKIN2, DIALOG_STYLE_PREVIEW_MODEL, "Skin Sec", string, "Sec", "Iptal");
			}
...
CMD:pide(playerid, params[])
{
    SendClientMessage(playerid, COLOR_ADM, "Pide yedin. (kuþbaþýlý)");
    return 1;
}